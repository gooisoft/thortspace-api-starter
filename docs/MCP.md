...
| `untangle(scope?, turnGroups=true, maxGroupMoveDegrees=10)` | **1.6.921** Remove unnecessary **path-step crossings** with the smallest changes that do it, keeping the layout you have: swap thorts between the slots their group already occupies, turn a group about its own axis, and nudge a group a few degrees. Unlike `optimise_layout` it does **not** re-place the groups, so it is safe on an arrangement whose positions carry meaning. Returns `crossingsBefore`/`crossingsAfter` plus `remaining` — the crossings it could not remove, named path by path, which usually means a group fanning out two ways that would be better split in two. |
